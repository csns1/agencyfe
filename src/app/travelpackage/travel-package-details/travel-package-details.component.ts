...
export class TravelPackageDetailsComponent implements OnInit {
  ...
  openPayment() {
    ...
    arr.forEach(e => {
      ...
      if (e.firstName == null || e.firstName.trim().length == 0) {
        this.toastr.error("Vendosni Emrin");
        return;
      } else if (e.lastName == null || e.lastName.trim().length == 0) {
        this.toastr.error("Vendosni Mbiemrin");
        return;
      } else if (e.age == null) {
        this.toastr.error("Vendosni Moshen");
        return;
      }

    });
    if (arr.length == 0) {
      this.toastr.error("Vendosni te pakten nje person.");
      return;
    }
  ...
  getNetet(dest: any) {
    if (dest.numberOfNights  == 0) {
      return ''
    } else {
      if (dest.numberOfNights  == 1) {
        return '(1 Nate)'
      } else {
        return "(" + dest.numberOfNights + " nete)"
      }
    }
  }
}
